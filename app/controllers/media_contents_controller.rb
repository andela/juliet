class MediaContentsController < ApplicationController
  before_action :connect_google_drive
...
private
  ...
  def filename
    @contact_file.split(".")[0..-2].join + Time.now.to_i + @file_ext
  end
  ...
  def save_to_drive(path)
    if @session.upload_from_file("#{path}", "#{filename}", convert: false)
      File.delete(path)
      redirect_to user_path(session[:user_id]) if @user.medias.create(file_name: filename)
    else
